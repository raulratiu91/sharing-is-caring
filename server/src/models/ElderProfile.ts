...
// Virtual fields for request counts
ElderProfileSchema.virtual('openRequestsCount').get(function () {
  return this.requests
    ? this.requests.filter((req) => req.status === 'open').length
    : 0;
});
...
ElderProfileSchema.virtual('urgentRequestsCount').get(function () {
  return this.requests
    ? this.requests.filter(
        (req) => req.status === 'open' && req.priority === 'high'
      ).length
    : 0;
});
...
// Export the model
export const ElderProfile = mongoose.model<IElderProfile>(
  'ElderProfile',
  ElderProfileSchema
);
